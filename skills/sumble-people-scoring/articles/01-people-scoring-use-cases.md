...
An account score answers "which companies." The next question — the one a rep
actually faces at 9am — is "who." Who at this account do I write to? Who gets
...
- **Lead routing and prioritization** — an inbound lead's score answers "AE
  now, nurture, or nowhere" in one lookup, and orders the SDR queue by fit ×
  engagement instead of recency of form fill.
- **Events** — who to invite, whose badge scan gets the first follow-up, and
  ...
  top-scored people at attending accounts, including the ones you've never
  met — who are precisely the reason to go.
...
Each of these usually runs on its own ad-hoc people logic — a title filter
here, a bought list there, a routing rule somebody wrote in 2023. One
...
engagement with you. What you usually *don't* know is who they are now —
current title, function, seniority, whether they still work there. Scoring
...
because there is none. They can absolutely be scored on fit — function,
seniority, skills, and the strength of the account around them.

Both populations get the same score, on the same scale, in the same list.
That's what lets a rep opening an account see the truth at a glance: "you have
touchpoints with three of the eleven people who matter here, and the top two
...
- **Function fit.** Is this person in a role you sell to — and how senior?
  ...
  are different conversations. The score interpolates by seniority *within*
  each function, so your true buyer's function outranks an adjacent one at
  every level.
...
- **Skills.** The tools on a person's profile, normalized against a technology
  ...
  it, evaluate it, or built their workflow around it. That's displacement
  knowledge no form fill reveals.
...
- **Engagement, where it exists.** First-party engagement — event attendance,
  product usage, email response — joins as a weighted factor for the people
...
Here's the piece most teams skip. A people list built for one campaign is
stale the day the campaign ends. The version that compounds is a **persistent
table in your data warehouse or CRM** — every person you could ever want to
target or know, scored, and **refreshed daily**.
...
move the morning after it happens — and you're the first vendor to know, not
the fifth.
...
every morning, and the only time you touch it again is when your ICP genuinely
changes.
...
- **One model.** Not one per persona, segment, and campaign. The interactions
  live in the features (function × seniority handles most of what per-persona
  models pretend to do); the variations live in how you slice the ranking.
...
Account scoring tells you where the revenue is. People scoring tells you who
to meet to go get it — including, especially, the people who have never heard
of you. The teams that do this stop asking "who should we reach out to?" in a
meeting every quarter. The answer is sitting in a column — and it was updated
this morning.
