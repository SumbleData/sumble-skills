...
The scoring and CRM skills build apps you run in a coding agent. This one is different: a **skill you install in your AI chat** that takes a single account from a name to who to reach, why now, and a drafted email — one conversation, one account at a time.

**Skill:** [`sumble-account-research`](../SKILL.md) — upload it to Claude, ChatGPT, or Gemini.

*Account scoring tells you [which companies](../../sumble-account-scoring/articles/01-account-score-should-tell-a-rep-what-to-do.md). People scoring tells you [who inside them](../../sumble-people-scoring/articles/01-people-scoring-use-cases.md). This skill is the motion that actually works one of those accounts end to end.*

## TLDR
- It's a **chat skill**, not a coding-agent skill — a single `SKILL.md` you upload to Claude (and adapt into a Custom GPT or a Gemini Gem). No local app, no terminal.
...
- The one thing it needs to actually *do* anything is the **Sumble MCP** connected. Without it, you get a good plan; with it, you get real people, signals, and emails.
- It builds a reusable **profile** of your company and sales plays once, then loads it on every later run — so setup happens a single time.

## What it does

Open a chat, name an account, and the skill walks the play a good rep runs but rarely has time for:

1. **Routes** — are you working a specific account, or brainstorming which to focus on? If brainstorming, it pulls your **CRM-synced account list** from Sumble (the auto-fresh one), and ranks the accounts by fit and *why each is compelling* — a sales play, a tech match, a fresh hiring signal.
...
- **It's a single Markdown file.** The entire skill is one `SKILL.md`. That makes it trivial to upload as a skill — or, on platforms without a skills feature, to paste straight in as custom instructions.
- **It's only as capable as its tool access.** The skill is instructions; the data comes from the **Sumble MCP**. On a surface that can reach Sumble, it runs the real motion. On one that can't, it degrades gracefully to drafting the plan and the emails from whatever you paste in. The requirement on every platform is the same: *can this chat call the Sumble MCP?*
...
- **It goes one account at a time.** Hand it a list and it researches them in turn, not in parallel — depth over a skim. You'll get a concrete hook on each account within seconds, before the deeper interview.
...
Most account research dies in a graveyard of browser tabs: you find the signal, you mean to write the email, the day moves on. This skill collapses find → decide → draft → send into one conversation, and it remembers your ICP so the second account is faster than the first and the tenth is faster than the second.
